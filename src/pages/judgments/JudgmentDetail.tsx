...
import { Calendar, Gavel, Download, Pencil, Users, FileText, Loader2, Copy, Check, Bookmark, BookmarkCheck, ChevronRight, Printer } from 'lucide-react';
...
const JudgmentDetail: React.FC = () => {
  ...
  const defaultTab = judgment.fullText ? 'text' : pdfUrl ? 'pdf' : 'metadata';
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-6 space-y-4 max-w-5xl">
        <nav className="flex items-center gap-1.5 text-sm text-muted-foreground">
          ...
          <ChevronRight className="h-3.5 w-3.5" />
          <span className="text-foreground truncate max-w-[40ch]">{judgment.caseName}</span>
        </nav>
        ...
        <div className="archive-card rounded-md p-6">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-2 border-t border-border pt-4">
            ...
            <Link to="/judgments">
              <Button variant="ghost" size="sm">{t('judgments.back')}</Button>
            </Link>
          </div>
        </div>
          ...
          <TabsContent value="text" className="mt-4">
            {judgment.fullText ? (
              ...
              <div className="archive-card rounded-md py-16 text-center">
                ...
                <p className="text-muted-foreground text-sm">{t('judgments.fulltext_unavailable')}</p>
                {pdfUrl && (
